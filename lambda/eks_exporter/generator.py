"""Orquestrador (Lambda): captura o cluster EKS na ORIGEM (via boto3) e gera, EM
MEMÓRIA, o Terraform modular (.tf + terraform.auto.tfvars.json + scripts/aws_lookup.py
+ CLUSTER-INFO.md). Devolve um dict {caminho_relativo: conteúdo}; o handler faz o
push desses arquivos no GitLab. A coleta/transformação é idêntica à do importer CLI.
"""
import base64
import json
import re
...
from .aws_boto3 import run_aws_command, _CAPTURE_FAILURES
...
from .report import render_cluster_info

# Caminho do helper real do data.external (lido do pacote e colocado em
# scripts/aws_lookup.py no commit — substitui o antigo blob base64 embutido).
_LOOKUP_SRC = Path(__file__).resolve().parent / "lookup" / "aws_lookup.py"

def generate_terraform_files(cluster_name, region, capture_full_vpc=True):
    """
    Gera, EM MEMÓRIA, a estrutura modular de Terraform (find-or-create, portável
    entre contas) para um cluster EKS existente. Devolve um dict
    {caminho_relativo: conteúdo_str} com TODOS os arquivos do módulo. Nada é escrito
    em disco: o chamador (lambda_handler) faz o commit no GitLab.
    """
    # profile permanece None dentro da Lambda: a coleta usa a role de execução da
    # função. Mantido para que TODAS as chamadas (run_aws_command(..., profile) e
    # collect_*(..., profile, run_aws_command)) fiquem idênticas à versão CLI.
    profile = None
    generated = {}
    # Zera o registro de comandos AWS que falharam (preenchido por run_aws_command).
    _CAPTURE_FAILURES.clear()
    # 1. Arquivos HCL estáticos -> dict
    for filename, content in TERRAFORM_FILES.items():
        generated[filename] = content.strip()
    print(f"✓ Arquivos .tf modulares gerados com sucesso.")
    # 1.1 Helper do data.external (scripts/aws_lookup.py), lido do pacote.
    # É o motor do find-or-create: roda em `terraform apply` (no runner de CI) e
    # consulta a conta de DESTINO via AWS CLI -- por isso continua CLI, não boto3.
    generated["scripts/aws_lookup.py"] = _LOOKUP_SRC.read_text()
    ...
    if not cluster_info:
        raise RuntimeError(
            f"Nao foi possivel obter informacoes do cluster '{cluster_name}' na regiao "
            f"'{region}'. Verifique o nome do cluster, a regiao e as permissoes da role.")
    ...
    if lt_user_data:
        for lt_id, ud_b64 in lt_user_data.items():
            ...
            generated[f"launch-templates-userdata/{lt_label}.userdata.txt"] = decoded
    ...
    # 4. Serializa o .tfvars.json -> dict
    generated["terraform.auto.tfvars.json"] = json.dumps(tfvars_data, indent=2)
    print(f"✓ Dados transformados (terraform.auto.tfvars.json).")
    # 4.5. Gera o relatorio CLUSTER-INFO.md em memoria.
    generated["CLUSTER-INFO.md"] = render_cluster_info(dict(
    ...
    print(f"✓ Relatorio CLUSTER-INFO.md gerado.")
